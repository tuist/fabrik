...
impl DaemonState {
    ...
    pub fn save(&self) -> Result<()> {
        ...
        // Write .fabrik/bazelrc for automatic Bazel configuration
        self.write_bazelrc()
            .context("Failed to write .fabrik/bazelrc")?;

        Ok(())
    }

    /// Write bazelrc file in the daemon state directory for Bazel integration
    /// This file is automatically loaded via BAZELRC environment variable
    pub fn write_bazelrc(&self) -> Result<()> {
        let state_dir = self.state_dir();

        // Generate bazelrc content
        let grpc_url = format!("grpc://127.0.0.1:{}", self.grpc_port);
        let bazelrc_content = format!(
            "# Auto-generated by Fabrik daemon\n\
             # This file is updated automatically when the daemon starts\n\
             # DO NOT EDIT - changes will be overwritten\n\
             #\n\
             # Loaded automatically via BAZELRC environment variable\n\
             # exported by Fabrik shell activation\n\
             #\n\
        ...
        // Write bazelrc file to state directory
        let bazelrc_path = state_dir.join("bazelrc");
        fs::write(&bazelrc_path, bazelrc_content)
            .with_context(|| format!("Failed to write bazelrc: {}", bazelrc_path.display()))?;

        Ok(())
    }

    /// Get the path to the auto-generated bazelrc file
    pub fn bazelrc_file(&self) -> PathBuf {
        self.state_dir().join("bazelrc")
    }
    ...
    pub fn generate_env_exports(&self, shell: &str) -> String {
        ...
        let grpc_url = format!("grpc://127.0.0.1:{}", self.grpc_port);
        let bazelrc_path = self.bazelrc_file();
        ...
        match shell {
            "fish" => {
                ...
                exports.push(format!("set -gx BAZELRC {}", bazelrc_path.display()));
            ...
            _ => {
                ...
                // Bazel configuration file
                exports.push(format!("export BAZELRC={}", bazelrc_path.display()));
